...
int32_t main() {
  ...
  spheres[6] = Sphere(16.5, Vector(27, 16.5, 47), Vector(), Vector(1, 1, 1) * .999, SPECular);//Mirr
  spheres[7] = Sphere(16.5, Vector(73, 16.5, 78), Vector(), Vector(1, 1, 1) * .999, REFRactive);//Glas
      ...
      for (int sy = 0, i = (h - y - 1) * w + x; sy < 2; sy++)     // 2x2 subpixel rows
        for (int sx = 0; sx < 2; sx++, r = Vector()) {        // 2x2 subpixel cols
          for (int s = 0; s < samps; s++) {
            double r1 = 2 * urd(mt_rand), dx = r1 < 1 ? sqrt(r1) - 1 : 1 - sqrt(2 - r1);
            double r2 = 2 * urd(mt_rand), dy = r2 < 1 ? sqrt(r2) - 1 : 1 - sqrt(2 - r2);
